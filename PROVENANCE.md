...
## MFC-J5720DW (second model; live probing, 2026-09-20)

Everything in this section comes from live probing of an MFC-J5720DW owned by
the contributor (the raw-protocol probe scripts under `probe/` are the tool -- `adf_probe.py` runs one job and dumps the reply, `adf_select_probe.py` compares select variants by their offer, `fake_j5720.py` replays a dump as a fake printer;
no vendor software was involved and no capture of the vendor driver exists
for this unit). The protocol, framing, and codecs above are identical on this
model; the rows below are the model differences found.

| Constant | Meaning | Source |
|---|---|---|
| Feeder select is `ESC S ADF` (`1b 53 0a 41 44 46 0a 80`), not `ESC D ADF` | The unit acks `ESC D ADF` with `0x80` but stays on the glass: the following `ESC I` offer is `300,300,2,213,2527,295,3484,` (flag 2, concrete height) and `ESC X` scans the (empty) flatbed. `ESC S ADF` switches it: the offer becomes `300,300,1,213,2527,0,0,` (flag 1, height 0) and `ESC X` feeds the sheet. Hence the offer's third field is the SOURCE the device settled on (1 = feeder, 2 = glass), consistent with the J6920DW's loaded/empty offers above | Live probe, `probe/adf_select_probe.py`: five select variants, one connection each, offer compared |
| Empty feeder is a lone `0xc2` in reply to `ESC X` | With `ESC S ADF` selected and no sheet sensed, the select ack is still `0x80` and the offer names the feeder; the `0xc2` (the same "empty" value the J6920DW returns at its `ESC D` ack) arrives only at `ESC X`, alone, in place of image data | Live probe, `probe/adf_probe.py` with an unseated sheet |
| Open-ended feeder JPEG: SOF height `65535`, no DNL, stream ends at the sheet | `A=0,0,2512,0` (the height-unknown request the J6920DW also gets) returns a baseline JPEG declaring height 65535 whose entropy data ends after the real sheet (3504 rows for A4), then a normal EOI and the usual `82 07 00 01 00 84 00 00 00 00` end-of-page marker. libjpeg refuses any dimension above 65500, so the page must be probe-decoded to find its row count and the SOF patched (`ResolveUnknownJpegHeight`) | Live probe capture (2 082 839 bytes, 32 chunks: 31 x `0xfff4` sentinel + one honest `0xc800`), replayed through `probe/fake_j5720.py` against `brscan-cli`. Not committed: it is the image of a personal document; the contributor holds it |
| Geometry at 300 dpi: glass 2527 x 3484 px; feeder 2527 px wide, corner-registered | Flatbed offer `2527,295,3484`; feeder offer `2527,0,0`; a fed A4 sheet's content spans the full 2512-px request width, so there is no centering slack in the feeder sensor (unlike the J6920DW's 3472-px sensor) | Live ESC I offers and the fed-sheet decode |
| Scan-button flatbed job parks at the panel's "Next page?" prompt after the end-of-page marker | Nothing follows the `82 07 00 01 00 84 00 00 00 00` marker until the user answers on the printer: No -> the job-final `0x80`; Yes -> the next page's block header after Start. With the 20 s scan timeout the completed page was discarded whenever the answer took longer; `kNextPageTimeoutMs` (5 min) now bounds that wait | `BRSCAN_TRACE=1` transport dump of a scan-button job (`ESC K` flow): 736 packets ending in the 10-byte marker, then silence until the panel was answered |
| Scan-button paper areas need re-centering for the 2527-px sensor | The captured table's centered rows (e.g. `LETTER {478,2990,3253}`) are framed for the 3472-px J6920DW sensor; sent as-is, the J5720DW returned the page with its left 478 px cut and the row wrapped at the sensor edge. `sensor_width = 2527` in the daemon config re-centers them (`RecenterAreaForSensor`) | Scan-button FILE job before/after the config key |
| Bonjour TXT: `ty=Brother MFC-J5720DW mfg=Brother mdl=MFC-J5720DW button=T feeder=T flatbed=T` on `_scanner._tcp` port 54921 | The `mdl` the ICA module's `DeviceMatchingInfo.plist` matches on | `dns-sd -L` |
...
| macOS-26 Image Capture builds each combined-PDF page (`Quartz PDFContext`) at the **selected paper size** and **centers** the delivered image on it (page content stream = `<pageWidth> 0 0 <imageHeightPt> 0 <(pageH−imgH)/2> cm … Do`); a delivered image SHORTER than the paper height gains a white margin split top+bottom, while an image at the full paper height fills the page | Why the ADF color trailing overscan is painted white at full height rather than cropped — a shorter/cropped image just re-pads to a page-background margin, so cropping cannot remove the band; only white-filling at full height does | Black-box observation: comparing this module's saved PDFs to the native driver's on the same document, reading each page's `MediaBox` and the image XObject `cm` transform. Carries no device identity or scan-image content |
| Past a short ADF sheet the color JPEG is padded to the requested height with uniform full-width mid-gray (`128`); below the paper's trailing edge the ADF also images its own near-white backing/roller. After a lone `0xc3` (jam) / `0x86` (Stop) status byte the device stays connected but **silent** for ~32 s until the host re-queries | Composition of the trailing "overscan" band the white-fill measures (gray pad + near-white backing), and why the lone-byte confirmation must use a short window (~2 s) rather than the 20 s scan timeout | Black-box observation of captured scans: `reference/c16-jam-imac.pcap` (the ~32 s post-`0xc3` silence) and a captured over-sized ADF color scan (the gray pad + backing rows). No scan-image content or device identity committed |
